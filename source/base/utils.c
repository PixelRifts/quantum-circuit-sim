...
void  U_PtrStackInit(U_PtrStack* stack, u32 cap) {
    stack->len = 0;
    stack->cap = cap;
    stack->elems = malloc(sizeof(void*) * cap);
    memset(stack->elems, 0, sizeof(void*) * cap);
}
...
void  U_PtrStackPush(U_PtrStack* stack, void* elem) {
    if (stack->len+1 > stack->cap) {
        stack->cap *= 2;
        stack->elems = realloc(stack->elems, sizeof(void*) * stack->cap);
    }
...
void* U_PtrStackPopSafe(U_PtrStack* stack) {
    if (stack->len > 0) return stack->elems[--stack->len];
    return nullptr;
}
