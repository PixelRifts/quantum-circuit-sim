...
f32 animate_f32exp(f32 val, f32 target, f32 speed, f32 dt, b8 instant) {
    if (instant) return target;
    return lerp(val, target, 1 - powf(speed, dt));
}

vec4 animate_vec4exp(vec4 val, vec4 target, f32 speed, f32 dt, b8 instant) {
    if (instant) return target;
    f32 t = 1 - powf(speed, dt);
    return v4(lerp(val.x, target.x, t),
              lerp(val.y, target.y, t),
              lerp(val.z, target.z, t),
              lerp(val.w, target.w, t));
}
