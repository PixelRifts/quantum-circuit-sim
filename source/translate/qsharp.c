                                ...
                                b8 is_adjoint)
{
    ...
    /* Diagnostic / classical-only intrinsics have no unitary effect and no
     * portable representation outside Q#. Preserve them as a comment instead
     * of a fake gate, so every backend (which already handles MQ_Stmt_Comment)
     * emits something syntactically safe and semantically inert. */
    static const char *qs_diag_calls[] = {
        "Message", "DumpMachine", "DumpRegister", "ResetAll", "Reset", NULL};
    for (int di = 0; qs_diag_calls[di]; di++)
    {
        if (strcmp((char *)callee_name.str, qs_diag_calls[di]) == 0)
        {
            string full_text = node_text(ctx->arena, call_node, ctx->src);
            return mq_stmt_comment(ctx->arena, full_text);
        }
    }
    ...
    /* is_cnot: for MQ_Gate_CX the two operands (ctrl, tgt) are already
     * stored as normal qubits[0..1] – no re-packing needed.  The old
     * re-packing code was specific to the MQ_Gate_X representation. */
...
static MQ_Stmt *lower_call_expr(QS_Ctx *ctx, TSNode call_node)
{
    ...
    if (node_is(callee, "identifier"))
    {
        ...
        /* ── Diagnostic / classical-only intrinsics ──────────────────────────
         *
         * Message, DumpMachine, DumpRegister, ResetAll, Reset have no portable
         * representation outside Q# (Reset also has no unitary representation).
         * Preserve them as a comment instead of a fake gate, so every backend
         * (which already handles MQ_Stmt_Comment) emits something syntactically
         * safe and semantically inert, rather than either (a) copying the call
         * through verbatim into a target language where the function doesn't
         * exist, or (b) being silently dropped.
         *
         * NOTE: this check must run here, before the generic "unknown
         * identifier -> user-defined routine call" fallback below, because
         * gate_from_name() returns MQ_Gate_Custom for any unrecognized name
         * (these calls included), so that fallback would otherwise catch them
         * first and this same check inside lower_gate_call() would never be
         * reached. */
        static const char *qs_diag_calls[] = {
            "Message", "DumpMachine", "DumpRegister", "ResetAll", "Reset", NULL};
        for (int di = 0; qs_diag_calls[di]; di++)
        {
            if (strcmp((char *)cname.str, qs_diag_calls[di]) == 0)
            {
                string full_text = node_text(ctx->arena, call_node, ctx->src);
                return mq_stmt_comment(ctx->arena, full_text);
            }
    ...
    if (is_for)
    {
        ...
        if (!node_null(pat_node))
        {
            TSNode id = node_is(pat_node, "identifier")
                            ? pat_node
                            : find_child(pat_node, "identifier");
            if (!node_null(id))
...
static void qs_emit_expr(FILE *out, MQ_Expr *e, b8 want_double)
{
    ...
    case MQ_Expr_Symbol:
    case MQ_Expr_Var:
        /* Normalize all pi variants → Q# Msk.PI() (from Microsoft.Quantum.Math as Msk) */
        if (e->name.str) {
